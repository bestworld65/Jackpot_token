...
pub mod jackpot_token {
    ...
    pub fn initialize(ctx: Context<Initialize>, token_mint: Pubkey, authority: Pubkey) -> Result<()> {
        ...
        state.pool_sol_amount = 10_000_000_000; // 10 SOL in lamports
    ...
    pub fn distribute(ctx: Context<Distribute>, holders: Vec<HolderInfo>) -> Result<()> {
        ...
        let sol_gained = (tokens_to_swap / 1_000_000) * 1_000_000_000; // In lamports
        ...
        let mut eligible_holders: Vec<&HolderInfo> = holders
            .iter()
            .filter(|h| h.amount >= 200_000)
            .collect();
        let mut jackpot_candidates: Vec<&HolderInfo> = holders
            .iter()
            .filter(|h| h.amount >= 400_000)
            .collect();

        if eligible_holders.is_empty() || jackpot_candidates.len() < 2 {
            return Err(ErrorCode::InsufficientHolders.into());
        }

        let holder_accounts = &ctx.remaining_accounts;
        if holder_accounts.len() != holders.len() {
            return Err(ErrorCode::InsufficientHolders.into());
        }

        let mut holder_account_map: Vec<(Pubkey, &AccountInfo)> = holders
            .iter()
            .zip(holder_accounts.iter())
            .map(|(holder, acc)| (holder.account, acc))
            .collect();
        ...
        **holder_account_map
            .iter()
            .find(|(pk, _)| *pk == winner1.account)
            .unwrap()
            .1
            .lamports
            .borrow_mut() += jackpot_per_winner;
        **ctx.accounts.authority.lamports.borrow_mut() -= jackpot_per_winner;
        **holder_account_map
            .iter()
            .find(|(pk, _)| *pk == winner2.account)
            .unwrap()
            .1
            .lamports
            .borrow_mut() += jackpot_per_winner;
...
pub struct Distribute<'info> {
    ...
    #[account(
        mut,
        constraint = authority.key() == state.authority @ ErrorCode::InvalidAuthority
    )]
